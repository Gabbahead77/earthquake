#### **earthquake_warning_app.py**

```python
# -*- coding: utf-8 -*-
"""
Erdbeben-/Vulkan-Frühwarnskript (Erweitert mit Konfigurationsdatei)

Dieses Skript lädt die Konfiguration aus der config.ini, 
ruft zwei RSS-Feeds (Griechenland & Italien) parallel ab und 
versendet bei Überschreitung eines Schwellenwertes Alarme per E-Mail und Telegram.
Zudem wird eine Weboberfläche mit Karte und Tabelle gestartet.
"""
...
############################################
import configparser
import requests  # HTTP-Datenabruf
import xml.etree.ElementTree as ET  # XML-Parsing
import smtplib  # E-Mail Versand
from email.mime.text import MIMEText  # E-Mail-Text erstellen
import telegram  # Telegram-Bot (pip install python-telegram-bot)
from flask import Flask, render_template_string  # Webserver
import time
import threading
import folium  # Karten
import datetime

############################################
# 2) KONFIGURATION AUS CONFIG.INI
############################################
...
############################################
# 3) DATENABRUF (RSS/XML) + PARSING
############################################
def fetch_quake_data(rss_url, feed_name):
    """
    Lädt den RSS-Feed von rss_url, parsed die Felder und fügt einen Schlüssel 'feed' hinzu.
    ...
    try:
        ...
        print(f"[FEHLER] Abruf fehlgeschlagen ({feed_name}): {e}")
        return []

    try:
        root = ET.fromstring(response.content)
    except ET.ParseError as e:
        print(f"[FEHLER] XML Parsing ({feed_name}): {e}")
        return []

    channel = root.find('channel')
    if channel is None:
        print(f"[WARN] Kein <channel>-Tag in Feed {feed_name}.")
        return []

    items = channel.findall('item')
    if not items:
        print(f"[INFO] Keine <item>-Tags in Feed {feed_name}.")
        return []
...
def send_email_alert(quake):
    subject = f"Alarm ({quake.get('feed','Unbekannt')}): M{quake['magnitude']:.1f}"
    body = (f"Ort: {quake['region']}\n"
            f"Magnitude: {quake['magnitude']}\n"
            f"Tiefe: {quake['depth']} km\n"
            f"Koordinaten: {quake['lat']}, {quake['lon']}\n"
            f"Zeit(UTC): {quake['time_utc']}\n"
            f"Link: {quake['link']}\n")
    ...
    try:
        ...
        print(f"[INFO] Alarm-Mail verschickt an {RECEIVER_EMAIL} ({quake.get('feed')})")
    except Exception as e:
        print(f"[FEHLER] E-Mail-Versand fehlgeschlagen: {e}")

############################################
# 5) TELEGRAM-BOT-VERSAND
############################################
def send_telegram_alert(quake):
    message = (f"Alarm ({quake.get('feed','Unbekannt')})!\n"
               f"Magnitude: {quake['magnitude']}\n"
               f"Tiefe: {quake['depth']} km\n"
               f"Ort: {quake['region']}\n"
               f"Koordinaten: {quake['lat']}, {quake['lon']}\n"
    ...
    try:
        ...
        print(f"[INFO] Telegram-Alarm gesendet an Chat {TELEGRAM_CHAT_ID} ({quake.get('feed')})")
    except Exception as e:
        print(f"[FEHLER] Telegram-Versand fehlgeschlagen: {e}")

############################################
# 6) WEBOBERFLÄCHE MIT FLASK
############################################
app = Flask(__name__)
quake_data = []
...
def index():
    ...
    <head>
        <meta charset="utf-8" />
        <title>Erdbeben-/Vulkansicht</title>
        <style>
          .container {
            display: flex;
            flex-direction: row;
    ...
    <body>
      <h1>Erdbeben & Vulkanausbrüche in Griechenland und Italien</h1>
      <div class="container">
        <div class="table-container">
          <h2>Letzte Ereignisse (max. {{max_list}})</h2>
          <table>
            <tr>
              <th>UTC-Zeit</th>
              <th>Magnitude</th>
...
############################################
# 7) FEED-PROZESSIERUNG IN PARALLEL
############################################
...
def process_feed(feed_name, rss_url):
    ...
    while True:
        print(f"[INFO] Hole neue Daten für {feed_name} ...")
        ...
        if quakes:
            ...
            if dt_obj:
                if (latest_time_utc[feed_name] is None) or (dt_obj > latest_time_utc[feed_name]):
                    print(f"[INFO] NEUES Ereignis für {feed_name} gefunden.")
                    ...
                    if top['magnitude'] >= MAG_THRESHOLD:
                        ...
                        if (now - last_email_time[feed_name]) > EMAIL_INTERVAL:
                            print(f"[INFO] Wiederholungs-Alarm (E-Mail) für {feed_name}")
                            send_email_alert(top)
                            last_email_time[feed_name] = now
                        if (now - last_telegram_time[feed_name]) > TELEGRAM_INTERVAL:
                            print(f"[INFO] Wiederholungs-Alarm (Telegram) für {feed_name}")
                            send_telegram_alert(top)
                            last_telegram_time[feed_name] = now
                    else:
                        print(f"[INFO] Gleiches Ereignis für {feed_name}, aber unter Grenzwert.")
                else:
                    print(f"[INFO] Top-Ereignis für {feed_name} ist älter als der letzte bekannte Zeitstempel.")
            else:
                print(f"[WARN] Konnte Zeitstempel für {feed_name} nicht parsen.")
        else:
            print(f"[INFO] Keine Daten für {feed_name} erhalten.")
...
############################################
# 8) HAUPTPROGRAMM / THREADS STARTEN
############################################
def start_flask_app():
